...
function getStyleInstruction(stylePreset = 'balancedModern') {
  switch (stylePreset) {
    case 'wallGrid':
      return 'Steer the styling toward a refined designer wall-led composition with compact geometric wall planters, supporting table or shelf companions, neat alignment, and clearly visible mounting support.';
    case 'hanging':
      return 'Steer the styling toward an airy upscale composition with one or two compact hanging planters only where the listed placements support them, plus grounded companion plants nearby with believable support.';
    case 'shelfStyling':
      return 'Steer the styling toward curated shelf, ledge, and tabletop styling with varied heights, layered but tidy spacing, and fully supported planters.';
    case 'cornerRetreat':
      return 'Steer the styling toward a calming biophilic corner retreat with one anchored floor plant and additional smaller companion plants on nearby surfaces while keeping walkways open.';
    default:
      return 'Steer the styling toward a balanced modern biophilic arrangement with editorial-quality staging, clean spacing, subtle variation in height, and a professional interior-design feel.';
  }
}

function getRealismInstruction(multiPlant = false) {
  if (multiPlant) {
    return 'Aim for a dense yet curated modern biophilic interior that looks like a professional editorial photograph: realistic mature foliage, elegant ceramic or matte planters, believable natural daylight, accurate lens perspective, and layered plant placement around shelves, desks, side tables, corners, floor stands, wall planters, and hanging supports wherever appropriate.';
  }

  return 'Aim for a highly realistic professional interior-design photo with believable scale, elegant planter materials, natural daylight, soft accurate shadows, and subtle editorial polish rather than CGI, collage, or illustration.';
}
...
function buildVisualizationPrompt({
  ...
  const realismInstruction = getRealismInstruction(renderStyle === 'same-room-multi-plant');
  if (renderStyle === 'same-room-multi-plant') {
    return `Edit this exact room photo and preserve the room as-is. Keep the existing layout, walls, floor, ceiling, furniture, decor, lighting, shadows, colors, and camera angle unchanged. Add only these healing plants: ${plantDescriptions}. Arrange them together as one cohesive, professionally styled, plant-rich composition that uses all listed plants and clearly follows the suggested placements from this list: ${plantDescriptions}. Distribute the listed plants across realistic supports such as corners, floor stands, desks, side tables, consoles, shelves, window ledges, wall-mounted planters, and hanging planters wherever those supports match the listed placements. ${styleInstruction} ${realismInstruction} The result should feel dense, elegant, realistic, and professionally styled, like a premium interior-design photo with many plants but still believable and calm. Layer the greenery so the room feels richly planted without becoming cluttered: one or two grounded anchor plants, several compact support plants on shelves or desks, and selective wall or hanging accents only where the room can physically support them. Keep each plant compact-to-medium relative to the room, vary heights naturally, preserve usable negative space, and avoid turning the room into a jungle wall or showroom installation. Every plant must physically rest on a real floor, shelf, table, ledge, sill, desk, or stand, or be visibly mounted to the wall or ceiling with realistic support hardware. Show realistic pot contact, leaf occlusion, grounding, and soft contact shadows so nothing looks pasted on or fake. If any plant is hanging, show the visible support attaching it to the ceiling. If any plant is wall-mounted, show a compact modern geometric planter support fixed to the wall. Do not duplicate plants or add extra planters beyond the listed plants. Do not make the cluster oversized, dominant, floating, unsupported, artificially glowing, collage-like, or centered as the main subject. Do not block walkways, large furniture areas, windows, doors, or important room features. Keep the arrangement refined, photorealistic, architecturally plausible, and close to a real luxury-home photo. Do not redesign the room or add unrelated extra objects. Photorealistic same-room interior multi-plant staging.`;
  }
  const subject = selectedPlantName ? `${selectedPlantName} in a modern planter` : `these healing plants: ${plantDescriptions}`;
  return `Edit this exact room photo and preserve the room as-is. Keep the existing layout, walls, floor, ceiling, furniture, decor, lighting, shadows, colors, and camera angle unchanged. Add only ${subject}. ${getPlacementInstruction(mode, placementText)} ${styleInstruction} ${realismInstruction} Match the original perspective, room scale, and lighting perfectly. The plant must feel beautifully staged for a calm modern healing interior, with subtle placement, believable contact shadows, realistic depth, and the polish of a professionally photographed designer room. The plant must physically rest on a real floor, shelf, table, ledge, sill, desk, or stand, or be visibly mounted to the wall or ceiling. If the placement is hanging, show the visible support attaching it to the ceiling. If the placement is wall-mounted, show a compact modern geometric planter support fixed to the wall. Keep the plant small-to-medium relative to the room and use at most one restrained planter installation for the selected plant. Do not make the plant oversized, dominant, floating, pasted-on, unsupported, or centered as the main subject. Do not block walkways, large furniture areas, windows, doors, or important room features. Keep the planter refined, minimal, and realistic. Do not redesign the room or add extra objects. Photorealistic same-room interior plant staging.`;
}
